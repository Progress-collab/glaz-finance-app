# PowerShell script for direct server execution with CD commands
Write-Host "=== RUNNING ON SERVER DIRECTLY (WITH CD) ===" -ForegroundColor Green
Write-Host "This script runs directly on Windows Server with full output" -ForegroundColor Cyan

Write-Host "`n1. NAVIGATING TO APPLICATION DIRECTORY:" -ForegroundColor Yellow
Write-Host "Current directory: $(Get-Location)" -ForegroundColor Cyan
Write-Host "Changing to C:\glaz-finance-app..." -ForegroundColor Cyan
...
Write-Host "`n3. CHECKING CURRENT STATUS:" -ForegroundColor Yellow
Write-Host "Current directory: $(Get-Location)" -ForegroundColor Cyan
...
Write-Host "`n5. CURRENT FILES:" -ForegroundColor Yellow
...
Write-Host "`n6. PUBLIC DIRECTORY:" -ForegroundColor Yellow
if (Test-Path "public") {
...
Write-Host "`n7. STOPPING ALL PROCESSES:" -ForegroundColor Yellow
...
Write-Host "`n8. FORCE GIT PULL:" -ForegroundColor Yellow
...
Write-Host "`n9. CHECKING FILES AFTER SYNC:" -ForegroundColor Yellow
Write-Host "Application files:" -ForegroundColor Cyan
Get-ChildItem "glaz-finance-app*.js" | ForEach-Object { 
    Write-Host "  $($_.Name) - $($_.LastWriteTime) - $($_.Length) bytes" 
}
...
Write-Host "`n10. INSTALLING DEPENDENCIES:" -ForegroundColor Yellow
...
if (Test-Path "glaz-finance-app-v2.js") {
    ...
    Write-Host "`n12. TESTING APPLICATION:" -ForegroundColor Yellow
    try {
    ...
    Write-Host "`n13. PM2 STATUS:" -ForegroundColor Yellow
    pm2 list
    
    Write-Host "`n14. PORT STATUS:" -ForegroundColor Yellow
