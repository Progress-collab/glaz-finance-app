...
if ($existingService) {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "Warning removing service: $result" -ForegroundColor Yellow
    }
...
try {
    ...
    Write-Host "Webhook server not responding yet (might still be starting)" -ForegroundColor Yellow
